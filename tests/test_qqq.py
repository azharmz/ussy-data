...
from update_qqq import validate, merge_incremental, audit_and_normalize_download, IDENTITY, POINTER


@unittest.skipUnless(importlib.util.find_spec('pandas'), 'pandas required')
class QQQTests(unittest.TestCase):
    def setUp(self):
        import pandas as pd
        self.pd = pd
        self.old = pd.DataFrame([dict(date=pd.Timestamp(d), security_id=IDENTITY, ticker='QQQ',
            open=100., high=102., low=99., close=101., adj_close=100., volume=1000)
            for d in ('2026-08-31', '2026-09-01')])

    def test_valid(self):
        validate(self.old)
        self.assertEqual(POINTER, 'benchmarks/QQQ/current.json')
    ...
    def test_no_overlap_or_duplicate_rejected(self):
        ...
        with self.assertRaises(ValueError):
            merge_incremental(self.old, new)
        with self.assertRaises(ValueError):
            validate(self.pd.concat([self.old, self.old]))
