...
class StooqRecoveryTest(unittest.TestCase):
    def test_normalizes_and_preserves_prices(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            f=read_stooq(p,"US0378331005","AAPL")
            self.assertEqual(list(f.columns),["date","security_id","ticker","open","high","low","close","adj_close","volume"])
            self.assertEqual(f.loc[0,"close"],339.75)
            self.assertEqual(f.loc[0,"adj_close"],339.75)
            self.assertEqual(int(f.loc[0,"volume"]),40711786)
