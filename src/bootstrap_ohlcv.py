...
def load_membership(s3, bucket: str, snapshot_date: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(records, list) or payload.get("count") != len(records):
        raise ValueError(f"Invalid membership payload at {key}")
...
def normalize_history(frame: pd.DataFrame, security_id: str, ticker: str) -> pd.DataFrame:
    ...
    normalized = normalized[OHLCV_COLUMNS].dropna(subset=["date", "open", "high", "low", "close"])
...
def upload_parquet(s3, bucket: str, key: str, frame: pd.DataFrame) -> None:
    ...
    if metadata.get("ContentLength", 0) <= 0:
        raise RuntimeError(f"Uploaded object is empty: {key}")
...
def run(args: argparse.Namespace) -> int:
    ...
    s3 = make_s3_client()
    membership = load_membership(s3, bucket, args.snapshot_date)
    selected = membership[args.start_index : args.start_index + args.batch_size]
    if not selected:
        raise ValueError(f"Batch starts beyond membership size ({len(membership)})")
    ...
    for position, record in enumerate(selected):
        ...
        if not args.force and object_exists(s3, bucket, key):
            ...
            results.append(Result(security_id, ticker, symbol, "skipped", object_key=key))
            continue
    ...
    manifest_key = f"backtest/manifests/bootstrap/{args.snapshot_date}/run-{run_id}-{attempt}-index-{args.start_index}.json"
    report = {
