"""Isolated QQQ benchmark: immutable versions + conditional pointer publication."""
import hashlib
import io
import json
import logging
...
def validate(frame):
    ...
    if set(COLS) - set(frame.columns) or frame.empty:
        raise ValueError('QQQ schema missing or empty')
    if set(frame['ticker']) != {'QQQ'} or set(frame['security_id']) != {IDENTITY}:
        raise ValueError('QQQ identity mismatch')
...
def audit_and_normalize_download(raw, out):
    ...
    if isinstance(flat.columns, pd.MultiIndex):
        if 'QQQ' not in flat.columns.get_level_values(1):
            raise ValueError('Unexpected Yahoo column layout')
        flat = flat.xs('QQQ', axis=1, level=1)
    ...
    if missing:
        raise ValueError(f'Missing Yahoo QQQ columns: {sorted(missing)}')
    ...
    if valid_source['date'].duplicated().any():
        raise ValueError('Yahoo returned duplicate valid QQQ dates')

    new = normalize_history(flat, IDENTITY, 'QQQ')
...
def main():
    ...
    out = Path('diagnostics/qqq')
    out.mkdir(parents=True, exist_ok=False)
    report = {'started_at': datetime.now(UTC).isoformat(), 'status': 'checking_existing',
              'ticker': 'QQQ', 'pointer_key': POINTER, 'git_sha': os.getenv('GITHUB_SHA'),
    ...
    try:
        ...
        try:
            ...
            if pointer.get('ticker') != 'QQQ' or not pointer.get('parquet_key', '').startswith(PREFIX):
                raise ValueError('Unrecognized QQQ pointer; review existing data')
            payload, _ = get(pointer['parquet_key'])
            if hashlib.sha256(payload).hexdigest() != pointer['sha256']:
                raise ValueError('Existing QQQ hash mismatch')
        ...
        if old is None:
            ...
            for page in s3.get_paginator('list_objects_v2').paginate(Bucket=bucket):
                for item in page.get('Contents', []):
                    ...
                    if 'QQQ' in parts and key.lower().endswith(('.parquet', '.json')):
                        candidates.append(key)
            report['existing_candidates'] = candidates
            if candidates:
                raise ValueError('Possible existing QQQ data found without official pointer; review before download')
            if os.getenv('GITHUB_EVENT_NAME') == 'schedule':
                report['status'] = 'awaiting_manual_initialization'
                save()
                print('QQQ not initialized. Run this workflow manually once.')
                return 0
        now = datetime.now(UTC)
        end = now.date() + (timedelta(days=1) if now.hour >= 22 else timedelta())
        params = dict(start='1999-03-10' if old is None else (pd.to_datetime(old.date).max().date() - timedelta(days=10)).isoformat(),
        ...
        if old is not None and result.equals(old[COLS].sort_values('date').reset_index(drop=True)):
            ...
            save()
            print('QQQ unchanged; existing pointer retained.')
            return 0
        buffer = io.BytesIO()
        result.to_parquet(buffer, index=False, compression='zstd')
        body = buffer.getvalue()
        key = PREFIX + uuid4().hex + '.parquet'
        manifest = dict(schema_version=1, ticker='QQQ', security_id=IDENTITY, role='market_benchmark_only',
        ...
        save()
        print('QQQ stopped. Inspect artifact; no universe or stock history changed.')
        return 1
