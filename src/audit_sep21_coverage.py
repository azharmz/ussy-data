...
for i,sid in enumerate(allowed,1):
    ...
    if i%200==0: print(f"SEP21_PROGRESS {i}/{len(allowed)} present={len(present)} absent={len(absent)}",flush=True)
print("SEP21_COVERAGE="+json.dumps({"eligible":len(eligible),"operational":len(oper),"allowed":len(allowed),"present":len(present),"absent":len(absent),"absent_ids":absent},sort_keys=True),flush=True)
